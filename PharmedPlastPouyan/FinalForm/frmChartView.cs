...
namespace PharmedPlastPouyan
{
    public partial class frmChartView : Form
    {
        ...
        private void frmChartView_Load(object sender, EventArgs e)
        {
            ...
            comTitle.DataSource = Utility.ToDataTable(DB.ChartTitles.Where(x=>x.ParentID==1).OrderBy(x=>x.Tartib).ToList());
            FirstRun = false;
        ...
        private DataTable ChartData(string strColumn, string TypeRes, string strRow, int NumberLine, string Code)
        {
            ...
            try
            {
                ...
                {
                    String[] Col2 = strColumn.Split('=');
                    string StopType = Col2[0];
                    for (int i = 0; i < Temp2.Length - 1; i++)//بازه زمانی
                    {
                        DataRow DR = Temp1.NewRow();
                        DR["Category"] = Temp2[i].ToString();
                        ...
                        {
                            double a = 0;
                            String[] Col = StopType.Split('+');
                            for (int u = 0; u < Col.Length; u++)//جمع کل تعداد ها
                            {
                                qwe = TypeRes + "(StopTime)";
                                if (strRow == "Day")
                                {
                                    String[] aaa = Temp2[i].Split('-');
                                    ewq = "Month=" + aaa[0] + " AND Day=" + aaa[1] + " AND ProdectionCode=" + Code2[x] + " And StopID = " + Col[u];
                                }
                                else
                                    ewq = strRow + " = " + Temp2[i] + " AND ProdectionCode=" + Code2[x] + " And StopID = " + Col[u];

                                string mm = DataStop.Compute(qwe, ewq).ToString();
                                if (mm != "")
                                    a += Convert.ToDouble(DataStop.Compute(qwe, ewq));
        ...
        {
            DataTable DT1;
            DT1 = ChartData(Data.SelectOneCol("ChartTitle", "ID", strlist, "Result"), Data.SelectOneCol("ChartTitle", "ID", strlist, "TypeResult"), SplitTime, send, temp);
        ...
        {
            DataTable DT2;
            DT2 = ChartData(Data.SelectOneCol("ChartTitle", "ID", strlist, "Result"), Data.SelectOneCol("ChartTitle", "ID", strlist, "TypeResult"), SplitTime, send, temp);
        ...
        {
            DataTable DT3;
            DT3 = ChartData(Data.SelectOneCol("ChartTitle", "ID", strlist, "Result"), Data.SelectOneCol("ChartTitle", "ID", strlist, "TypeResult"), SplitTime, send, temp);
        ...
        {
            DataTable DT4;
            DT4 = ChartData(Data.SelectOneCol("ChartTitle", "ID", strlist, "Result"), Data.SelectOneCol("ChartTitle", "ID", strlist, "TypeResult"), SplitTime, send, temp);
        ...
        {
            LINQDataContext DB = new LINQDataContext();
            DataTable dt =  Utility.ToDataTable(DB.ChartTitles.Where(x => x.ParentID == int.Parse(comTitle.SelectedValue.ToString())).OrderBy(x => x.Tartib).ToList());
        ...
        private void comchart_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if ((int)a["ParentID"] == 3)
            {
                ...
                LINQDataContext DB = new LINQDataContext();
//                comStop.DataSource = Data.Select("select * from ChartTitle where ParentID=" + comchart.SelectedValue + " order by Tartib ASC");
                comStop.DataSource = Utility.ToDataTable(DB.ChartTitles.Where(x=>x.ParentID==int.Parse(comchart.SelectedValue.ToString())).OrderBy(x=>x.Tartib).ToList());
        ...
        private void FilterChart()
        {
            ...
            if (FilterSort[1] == '1')
            {
                if (MultiSelect)
                    SqlWhere += " AND";
                MultiSelect = true;
                if (txtMachineNum.Text != "")
                {
                    SqlWhere += " MachineNum=" + txtMachineNum.Text;
                }
                else
                {
                    MessageBox.Show("بعد از اعمال فیلتر دستگاه حتما باید شماره دستگاه وارد شود", "اخطار", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    //txtMachineNum.Focus();
                    return;
                }
            }
            ...
            if (FilterSort[4] == '1')
            {
                ...
                {
                    MessageBox.Show("شماره لات نامبر باید 5 رقمی باشد", "اخطار", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    //txtLotStart.Focus();
                    return;
                }
            ...
            SqlWhere += " order by IDPR";
            Sql = "select * from ProductionData {0}";
            Sql = string.Format(Sql, SqlWhere);
            DataAll = Data.Select(Sql);
            Sql = "select ProductionData.Day,ProductionData.Month,ProductionData.ProdectionCode,ProductionData.WeekNum,Stop.IDProtection,StopType.Title,Stop.StopID,Stop.StopTime,tblDescription.Description FROM ProductionData ,Stop,tblDescription,StopType {0} and Stop.IDProtection= ProductionData.IDPR and tblDescription.IDPR=ProductionData.IDPR and StopType.StopID=Stop.StopID";
            Sql = string.Format(Sql, SqlWhere);
            Sql = Sql.Replace(" order by IDPR", "");
            Sql += " order by IDProtection";
            DataStop = Data.Select(Sql);

            DataForTable = FillDataForTable(DataAll, DataStop);
            return;
        ...
        {
            DataTable Temp1 = new DataTable();
            Temp1.Columns.Add("IDPR");
            Temp1.Columns.Add("Day");
            Temp1.Columns.Add("Month");
            Temp1.Columns.Add("Year");
            Temp1.Columns.Add("Shift");
            Temp1.Columns.Add("WeekNum");
            Temp1.Columns.Add("BatchNum");
            Temp1.Columns.Add("LotNum");
            Temp1.Columns.Add("MachineNum");
            Temp1.Columns.Add("WorkOrder");
            Temp1.Columns.Add("FastCode");
            Temp1.Columns.Add("ProdectionCode");
            Temp1.Columns.Add("ProdectionName");
            Temp1.Columns.Add("TemplateNum");
            Temp1.Columns.Add("Kaviteh");
            Temp1.Columns.Add("TypeOfTolid");
            Temp1.Columns.Add("PartNum");
            Temp1.Columns.Add("ControlNum");
            Temp1.Columns.Add("WastageNum");
            Temp1.Columns.Add("VaznRahgah");
            Temp1.Columns.Add("VaznKoloukheh");
            Temp1.Columns.Add("ActiveKaviteh");
            Temp1.Columns.Add("CycleTime");
            Temp1.Columns.Add("OperatorCode");
            Temp1.Columns.Add("OperatorName");
            Temp1.Columns.Add("GranuleSeries");
            Temp1.Columns.Add("ICCNum");
            Temp1.Columns.Add("OperatorOrderCode");
            Temp1.Columns.Add("OperatorOrderName");
            Temp1.Columns.Add("StopID1");
            Temp1.Columns.Add("StopTitle1");
            Temp1.Columns.Add("StopTime1");
            Temp1.Columns.Add("StopID2");
            Temp1.Columns.Add("StopTitle2");
            Temp1.Columns.Add("StopTime2");
            Temp1.Columns.Add("StopID3");
            Temp1.Columns.Add("StopTitle3");
            Temp1.Columns.Add("StopTime3");
            Temp1.Columns.Add("StopID4");
            Temp1.Columns.Add("StopTitle4");
            Temp1.Columns.Add("StopTime4");
            Temp1.Columns.Add("StopID5");
            Temp1.Columns.Add("StopTitle5");
            Temp1.Columns.Add("StopTime5");
            Temp1.Columns.Add("Description");
            return Temp1;
        }
        private DataTable FillDataForTable(DataTable All, DataTable Stop)
        {
            DataTable temp = CreateTable();
            for (int i = 0; i < All.Rows.Count; i++)//همه داده های فیلتر شده
            {
                DataRow DR = temp.NewRow();
                DR["IDPR"] = All.Rows[i]["IDPR"];
                DR["Day"] = All.Rows[i]["Day"];
                DR["Month"] = All.Rows[i]["Month"];
                DR["Year"] = All.Rows[i]["Year"];
                if ((int)All.Rows[i]["Shift"] == 1)
                    DR["Shift"] = "روز";
                else
                    DR["Shift"] = "شب";
                DR["WeekNum"] = All.Rows[i]["WeekNum"];
                DR["BatchNum"] = All.Rows[i]["BatchNum"];
                DR["LotNum"] = All.Rows[i]["LotNum"];
                DR["MachineNum"] = All.Rows[i]["MachineNum"];
                DR["WorkOrder"] = All.Rows[i]["WorkOrder"];
                DR["FastCode"] = All.Rows[i]["FastCode"];
                DR["ProdectionCode"] = All.Rows[i]["ProdectionCode"];
                DR["ProdectionName"] = All.Rows[i]["ProdectionName"];
                DR["TemplateNum"] = All.Rows[i]["TemplateNum"];
                if ((int)All.Rows[i]["TypeOfTolid"] == 1)
                    DR["TypeOfTolid"] = "تولید روتین";
                else
                {
                    if ((int)All.Rows[i]["TypeOfTolid"] == 2)
                        DR["TypeOfTolid"] = "تولید آزمایشی";
                    else
                        DR["TypeOfTolid"] = "راه اندازی";
                }
                DR["PartNum"] = All.Rows[i]["PartNum"];
                DR["ControlNum"] = All.Rows[i]["ControlNum"];
                DR["WastageNum"] = All.Rows[i]["WastageNum"];
                DR["VaznRahgah"] = All.Rows[i]["VaznRahgah"];
                DR["VaznKoloukheh"] = All.Rows[i]["VaznKoloukheh"];
                DR["ActiveKaviteh"] = All.Rows[i]["ActiveKaviteh"];
                DR["CycleTime"] = All.Rows[i]["CycleTime"];
                DR["OperatorCode"] = All.Rows[i]["OperatorCode"];
                DR["OperatorName"] = All.Rows[i]["OperatorName"];
                DR["GranuleSeries"] = All.Rows[i]["GranuleSeries"];
                DR["ICCNum"] = All.Rows[i]["ICCNum"];
                DR["OperatorOrderCode"] = All.Rows[i]["OperatorOrderCode"];
                DR["OperatorOrderName"] = All.Rows[i]["OperatorOrderName"];

                DataRow[] d = DataStop.Select("IDProtection=" + ((int)All.Rows[i]["IDPR"]).ToString());
                for (int j = 0; j < d.Length; j++)
                {
                    DR["Description"] = d[j]["Description"];
                    DR["StopID" + (j + 1)] = d[j]["StopID"];
                    DR["StopTitle" + (j + 1)] = d[j]["Title"];
                    DR["StopTime" + (j + 1)] = d[j]["StopTime"];
        ...
        private void TableView()
        {
            viewTable = true;
            SizeTableFix();
            if (radioTBLAll.Checked)
            {
                tblView.Columns["TypeOfTolid"].IsVisible = true;
                tblView.Columns["PartNum"].IsVisible = true;
                tblView.Columns["ControlNum"].IsVisible = true;
                tblView.Columns["WastageNum"].IsVisible = true;
                tblView.Columns["VaznRahgah"].IsVisible = true;
                tblView.Columns["VaznKoloukheh"].IsVisible = true;
                tblView.Columns["ActiveKaviteh"].IsVisible = true;
                tblView.Columns["CycleTime"].IsVisible = true;
                tblView.Columns["OperatorCode"].IsVisible = true;
                tblView.Columns["OperatorName"].IsVisible = true;
                tblView.Columns["GranuleSeries"].IsVisible = true;
                tblView.Columns["ICCNum"].IsVisible = true;
                tblView.Columns["OperatorOrderCode"].IsVisible = true;
                tblView.Columns["OperatorOrderName"].IsVisible = true;


                tblView.Columns["StopID1"].IsVisible = true;
                tblView.Columns["StopTitle1"].IsVisible = true;
                tblView.Columns["StopTime1"].IsVisible = true;
                tblView.Columns["StopID2"].IsVisible = true;
                tblView.Columns["StopTitle2"].IsVisible = true;
                tblView.Columns["StopTime2"].IsVisible = true;
                tblView.Columns["StopID3"].IsVisible = true;
                tblView.Columns["StopTitle3"].IsVisible = true;
                tblView.Columns["StopTime3"].IsVisible = true;
                tblView.Columns["StopID4"].IsVisible = true;
                tblView.Columns["StopTitle4"].IsVisible = true;
                tblView.Columns["StopTime4"].IsVisible = true;
                tblView.Columns["StopID5"].IsVisible = true;
                tblView.Columns["StopTitle5"].IsVisible = true;
                tblView.Columns["StopTime5"].IsVisible = true;
                tblView.Columns["Description"].IsVisible = true;
            }
            else
            {
                if (radioTBLStop.Checked)
                {
                    tblView.Columns["TypeOfTolid"].IsVisible = false;
                    tblView.Columns["PartNum"].IsVisible = false;
                    tblView.Columns["ControlNum"].IsVisible = false;
                    tblView.Columns["WastageNum"].IsVisible = false;
                    tblView.Columns["VaznRahgah"].IsVisible = false;
                    tblView.Columns["VaznKoloukheh"].IsVisible = false;
                    tblView.Columns["ActiveKaviteh"].IsVisible = false;
                    tblView.Columns["CycleTime"].IsVisible = false;
                    tblView.Columns["OperatorCode"].IsVisible = false;
                    tblView.Columns["OperatorName"].IsVisible = false;
                    tblView.Columns["GranuleSeries"].IsVisible = false;
                    tblView.Columns["ICCNum"].IsVisible = false;
                    tblView.Columns["OperatorOrderCode"].IsVisible = false;
                    tblView.Columns["OperatorOrderName"].IsVisible = false;
                ...
                else
                {
                    ...
                    tblView.Columns["StopID1"].IsVisible = false;
                    tblView.Columns["StopTitle1"].IsVisible = false;
                    tblView.Columns["StopTime1"].IsVisible = false;
                    tblView.Columns["StopID2"].IsVisible = false;
                    tblView.Columns["StopTitle2"].IsVisible = false;
                    tblView.Columns["StopTime2"].IsVisible = false;
                    tblView.Columns["StopID3"].IsVisible = false;
                    tblView.Columns["StopTitle3"].IsVisible = false;
                    tblView.Columns["StopTime3"].IsVisible = false;
                    tblView.Columns["StopID4"].IsVisible = false;
                    tblView.Columns["StopTitle4"].IsVisible = false;
                    tblView.Columns["StopTime4"].IsVisible = false;
                    tblView.Columns["StopID5"].IsVisible = false;
                    tblView.Columns["StopTitle5"].IsVisible = false;
                    tblView.Columns["StopTime5"].IsVisible = false;
                    tblView.Columns["Description"].IsVisible = false;
                }
            }
            tblView.DataSource = DataForTable;
